...
class Product_features extends Admin_controller {
  ...
  public function __construct () {
    ...
    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (4, 0)) && ($this->obj = ProductFeature::find_by_id ($id))))
  ...
  public function index ($id, $offset = 0) {
    ...
    $limit = 25;
    $total = ProductFeature::count (array ('conditions' => $conditions));
    $objs = ProductFeature::find ('all', array ('offset' => $offset < $total ? $offset : 0, 'limit' => $limit, 'order' => 'id DESC', 'conditions' => $conditions));
  ...
  public function create () {
    ...
    if (!ProductFeature::transaction (function () use (&$obj, $posts, $name) { return verifyCreateOrm ($obj = ProductFeature::create (array_intersect_key ($posts, ProductFeature::table ()->columns))) && $obj->name->put ($name); }))
  ...
  public function update () {
    ...
    if (!ProductFeature::transaction (function () use ($obj, $posts, $name) { if (!$obj->save () || ($name && !$obj->name->put ($name))) return false; return true; }))
  ...
  public function destroy () {
    ...
    if (!ProductFeature::transaction (function () use ($obj) { return $obj->destroy (); }))
  ...
  public function sort ($tag_id, $obj_id, $sort) {
    ...
    if (!ProductFeature::transaction (function () use ($conditions, $obj, $sort, &$change) { if (($next = ProductFeature::find ('one', array ('conditions' => $conditions))) && array_push ($change, array ('id' => $next->id, 'old' => $next->sort, 'new' => $sort))) { $next->sort = $sort; if (!$next->save ()) return false; } return $obj->save (); }))
      return redirect_message (array ($this->uri_1, $parent->id, $this->uri_2), array ('_flash_danger' => '排序失敗！'));
  ...
  private function _validation_create (&$posts, &$name) {
    if (!isset ($name)) return '沒有選擇 圖片！';
    if (!is_upload_image_format ($name, 20 * 1024 * 1024, array ('gif', 'jpeg', 'jpg', 'png'))) return '圖片 格式錯誤！';
  ...
  private function _validation_update (&$posts, &$name, $obj) {
    if (!((string)$obj->name || isset ($name))) return '沒有選擇 圖片！';
    if ($name && !is_upload_image_format ($name, 20 * 1024 * 1024, array ('gif', 'jpeg', 'jpg', 'png'))) return '圖片 格式錯誤！';
