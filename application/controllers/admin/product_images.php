...
class Product_images extends Admin_controller {
  ...
  public function __construct () {
    ...
    if (in_array ($this->uri->rsegments (2, 0), array ('edit', 'update', 'destroy', 'sort')))
      if (!(($id = $this->uri->rsegments (4, 0)) && ($this->obj = ProductImage::find_by_id ($id))))
  ...
  public function index ($id, $offset = 0) {
    ...
    $limit = 25;
    $total = ProductImage::count (array ('conditions' => $conditions));
    $objs = ProductImage::find ('all', array ('offset' => $offset < $total ? $offset : 0, 'limit' => $limit, 'order' => 'id DESC', 'conditions' => $conditions));
  ...
  public function create () {
    ...
    if (!ProductImage::transaction (function () use (&$obj, $posts, $name) { return verifyCreateOrm ($obj = ProductImage::create (array_intersect_key ($posts, ProductImage::table ()->columns))) && $obj->name->put ($name); }))
  ...
  public function update () {
    ...
    if (!ProductImage::transaction (function () use ($obj, $posts, $name) { if (!$obj->save () || ($name && !$obj->name->put ($name))) return false; return true; }))
  ...
  public function destroy () {
    ...
    if (!ProductImage::transaction (function () use ($obj) { return $obj->destroy (); }))
  ...
  public function sort ($tag_id, $obj_id, $sort) {
    ...
    if (!ProductImage::transaction (function () use ($conditions, $obj, $sort, &$change) { if (($next = ProductImage::find ('one', array ('conditions' => $conditions))) && array_push ($change, array ('id' => $next->id, 'old' => $next->sort, 'new' => $sort))) { $next->sort = $sort; if (!$next->save ()) return false; } return $obj->save (); }))
      return redirect_message (array ($this->uri_1, $parent->id, $this->uri_2), array ('_flash_danger' => '排序失敗！'));
